...
def test_fluxo_integração_completo():
    ...
    produtos_filial_u = UtilidadesDomesticas.objects.filter(
        filial_id=filial_id
    )
    ...
    assert not Filial.objects.filter(
        id_filial=filial_id
    ).exists()
    assert not Filial.objects.filter(
        id_filial=filial_id
    ...
    assert not UtilidadesDomesticas.objects.filter(
        filial_id=filial_id
    ).exists()
